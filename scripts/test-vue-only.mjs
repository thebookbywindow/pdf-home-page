...
const canonicalRoot = path.join(root, "en", "pdf-tools");
const forbidden = [
  "tools",
  "partials",
  "src/legacy",
...
assert.ok(fs.existsSync(canonicalRoot), "Vue MPA must retain the canonical tool root.");
...
assert.ok(toolDirectories.length > 0, "Canonical tool pages must be generated.");
for (const slug of toolDirectories) {
  assert.ok(
    fs.existsSync(path.join(canonicalRoot, slug, "index.html")),
    `${slug} must expose only its canonical Vue entry.`
  );
}
...
assert.match(generatorSource, /canonicalToolsDir/);
...
console.log(`PASS Vue-only layout contains ${toolDirectories.length} canonical tool pages.`);
